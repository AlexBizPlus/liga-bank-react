...
const Calculator = observer(({ onSuccess }: ICalculator) => {
  ...
  const [isShowMinLoanMessage, setIsShowMinLoanMessage] = useState<boolean>(
    false
  );
  ...
  useEffect(() => {
    ...
    if (store.currentInitialFee > store.maxInitialFee)
      store.currentInitialFee = store.maxInitialFee;
    if (store.currentInitialFee < store.minInitialFee)
      store.currentInitialFee = store.minInitialFee;
    setIsShowQueryForm(false);
    store.updateLoanСonditions();
    ...
    <section className={cl(s.wrap)}>
      <div className={cl(s.content)} id="calculator">
        ...
        <form className={cl(s.form)}>
          ...
          {store.currentLoanType && (
            <>
              <h3 className={cl(s.stepCaption, s.stepCaption__marginTop)}>
              ...
              <InputTypeRange
                ...
                minRangeText={`${store.minInitialFeePercent}% = ${splitPrice(
                  store.minInitialFee
                )}`}
                maxRangeText={splitPrice(store.maxInitialFee.toString())}
                showExceedMaxValue={(value: boolean) =>
                  setIsShowMinLoanMessage(value)
                }
              />
              <InputTypeRange
                units={"лет"}
              ...
              {store.discountCheckboxes.length > 0 &&
                store.discountCheckboxes.map((item, i) => {
                  return (
                    <Checkbox
              ...
              {store.insuranceCheckboxNames.length > 0 &&
                store.insuranceCheckboxNames.map((item, i) => {
                  return (
                    <Checkbox
                      className={cl(s.checkbox)}
                      labelText={item}
        ...
        {store.currentLoanType && isShowMinLoanMessage && (
          <div className={cl(s.minLoanMessage)}>
            <p className={cl(s.minLoanMessageCaption)}>
              Наш банк не выдаёт {store.offerMessageCaption || "кредиты"} меньше {splitPrice(store.minCreditAmount)} рублей.
            </p>
            <p className={cl(s.minLoanMessageText)}>
