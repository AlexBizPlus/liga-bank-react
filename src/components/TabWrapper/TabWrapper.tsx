...
const TabWrapper: React.FC<ITabsProps> = (prop) => {
  ...
  const handleTouchEnd: TouchEventHandler<HTMLDivElement> = (evt) => {
    ...
    if (coordX && evt.changedTouches[0].clientX < coordX) setActiveTab(activeTab < tabs.length -1 ? activeTab + 1 : activeTab)
  };

  const setTabIcon = (title: string) => {
    let icon, iconWidth, iconHeight;
  ...
  const addTabImg = (tab: ITab) => {
    ...
    return Object.assign({}, tab.component, { img: img });
  };
  ...
  return (
    <div className={cl(s.tabWrap)} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <TabComponent
        component={addTabImg(tabs[activeTab])}
      ...
      {tabs.length !== 0 && (
        <div className={cl(s.paggination)}>
          {tabs.map((item, i) => {
            return (
              <div
                className={cl(s.dot, { [s.dot__active]: i === activeTab })}
                key={`item${i}`}
              />
            );
          })}
