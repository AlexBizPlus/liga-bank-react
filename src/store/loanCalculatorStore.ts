...
export default class LoanCalculatorStore {
  ...
  setRequiredMonthlyIncome() {
    this.requiredMonthlyIncome =
      (this.monthlyPayment * 100) / REQUIRED_MONTHLY_INCOME_PERCENT;
  }
  ...
  setInterestRate() {
    ...
    if (this.borderInterestRate > 0) {
      switch (this.currentInsuranceDiscounts) {
        case 0:
          this.interestRate =
            this.currentPrice < this.borderInterestRate
              ? this.maxInterestRate
              : this.minInterestRate;
          break;
  ...
  setMaxInitialFee() {
    this.maxInitialFee =
      this.currentPrice - this.discount - this.minCreditAmount;
  }

  setInitialLoanСonditions() {
    let conditions = Loans.find((item) => item.name === this.loanType);
    if (conditions) {
      ...
      this.discountCheckboxes = conditions.discountCheckboxes
        ? conditions.discountCheckboxes
        : [];
      this.creditInputTitle = conditions.creditInputTitle
        ? conditions.creditInputTitle
        : "";
      this.step = conditions.step ? conditions.step : 0;
      this.minInitialFeePercent = conditions.minInitialFee
        ? conditions.minInitialFee
        : 0;
      this.minCreditAmount = conditions.minCreditAmount
        ? conditions.minCreditAmount
        : 0;
      this.setMinInitialFee();
      this.setMaxInitialFee();
      this.currentInitialFee = this.minInitialFee;
      this.stepInitialFeePercent = conditions.stepInitialFee
        ? conditions.stepInitialFee
        : 0;
      ...
      this.offerCaption = conditions.offerCaption
        ? conditions.offerCaption
        : "";
      this.setLoanOffer();
      this.minInterestRate = conditions.minInterestRate
        ? conditions.minInterestRate
        : 0;
      this.maxInterestRate = conditions.maxInterestRate
        ? conditions.maxInterestRate
        : 0;
      this.borderInterestRatePercent = conditions.borderInterestRatePercent
        ? conditions.borderInterestRatePercent
        : 0;
      this.borderInterestRate = conditions.borderInterestRate
        ? conditions.borderInterestRate
        : 0;
      this.setInterestRate();
      this.setMonthlyPayment();
      this.insuranceCheckboxNames = conditions.insuranceCheckboxNames
        ? conditions.insuranceCheckboxNames
        : [];
      this.allSpecialInterestRate = conditions.allSpecialInterestRate
        ? conditions.allSpecialInterestRate
        : 0;
      this.specialInterestRate = conditions.specialInterestRate
        ? conditions.specialInterestRate
        : 0;
      this.offerMessageCaption = conditions.offerMessageCaption
        ? conditions.offerMessageCaption
        : "";
      this.discount = 0;
